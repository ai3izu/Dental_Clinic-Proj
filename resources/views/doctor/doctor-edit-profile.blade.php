...
@section('content')
    <div
        ...
        <form method="POST" action="{{ route('doctor.profile.update') }}" class="space-y-5" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="lekarz o imieniu osiągnął.."
                    ...
                    aria-label="Opis kariery zawodowej i doświadczenia dentysty">{{ old('description', $doctor->description ?? '') }}</textarea>
                @error('description')
            ...
            <div class="pt-3 flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0">
                <button type="submit"
                    class="bg-[#28A745] hover:bg-[#218838] text-white font-semibold px-5 py-2 rounded-lg shadow text-sm sm:text-base">
                    Zapisz zmiany
                </button>
