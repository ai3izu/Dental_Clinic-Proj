    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="my-2">
            <form action="{{ route('admin.dashboard') }}" method="GET" class="flex flex-col md:flex-row gap-2">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-lg whitespace-nowrap w-full md:w-auto">
                    Szukaj
                </button>
            </form>
        </div>
                    ...
                    <tr class="border-b">
                        ...
                        <td class="py-2 px-4">
                            ...
                            <form action="{{ route('admin.doctors.destroy', $doctor->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 ml-4" onclick="return confirm('Czy na pewno chcesz usunąć tego dentystę?')">Usuń</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
            ...
            <tbody>
                @foreach ($appointments ?? [] as $appointment)
                    <tr class="border-b">
                        ...
                        <td class="py-2 px-4">{{ $appointment->patient->user->first_name }} {{ $appointment->patient->user->last_name }}</td>
                        <td class="py-2 px-4">{{ $appointment->doctor->user->first_name }} {{ $appointment->doctor->user->last_name }}</td>
                        ...
                        <td class="py-2 px-4">
                            ...
                            <form action="{{ route('admin.appointments.destroy', $appointment->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 ml-4" onclick="return confirm('Czy na pewno chcesz usunąć tę wizytę?')">Usuń</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
        ...
        <table class="min-w-full text-sm text-[#13293D] border mb-4">
            <thead class="bg-[#3E92CC] text-white">
                <tr>
                    <th class="py-2 px-4 text-left">Pacjent</th>
                    <th class="py-2 px-4 text-left">Dentysta</th>
                    <th class="py-2 px-4 text-left">Ocena</th>
                    <th class="py-2 px-4 text-left">Komentarz</th>
                    <th class="py-2 px-4 text-left">Akcje</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($reviews ?? [] as $review)
                    <tr class="border-b">
                        <td class="py-2 px-4">{{ $review->patient->user->first_name }} {{ $review->patient->user->last_name }}</td>
                        ...
                        <td class="py-2 px-4">
                            ...
                            <form action="{{ route('admin.reviews.destroy', $review->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 ml-4" onclick="return confirm('Czy na pewno chcesz usunąć tę opinię?')">Usuń</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
        ...
        <table class="min-w-full text-sm text-[#13293D] border mb-4">
            <thead class="bg-[#3E92CC] text-white">
                <tr>
                    ...
                    <th class="py-2 px-4 text-left">Data</th>
                    <th class="py-2 px-4 text-left">Metoda płatności</th>
                    <th class="py-2 px-4 text-left">Akcje</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($transactions ?? [] as $transaction)
                    <tr class="border-b">
                        <td class="py-2 px-4">{{ $transaction->patient->user->first_name }} {{ $transaction->patient->user->last_name }}</td>
                        ...
                        <td class="py-2 px-4">
                            <a href="{{ route('admin.transactions.edit', $transaction->id) }}" class="text-blue-500 ml-4">Edytuj</a>
                            <form action="{{ route('admin.transactions.destroy', $transaction->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 ml-4" onclick="return confirm('Czy na pewno chcesz usunąć tę transakcję?')">Usuń</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
