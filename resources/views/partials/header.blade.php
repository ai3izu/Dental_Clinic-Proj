    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-3 md:py-4">
            ...
            <div class="flex items-center space-x-2">
                ...
                <span class="text-primary text-xl lg:text-2xl font-bold">
                    DentMax @auth {{ Auth::user()->role }} @endauth
                </span>
            </div>
            ...
            <div class="hidden lg:flex items-center space-x-6">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="bg-primary hover:bg-primaryDark text-white font-semibold px-4 py-2 rounded-lg shadow-md transition duration-300 text-sm lg:text-base">Logowanie</a>
                    <a href="{{ route('register') }}"
                        class="bg-white border border-primary text-primary font-semibold px-4 py-2 rounded-lg shadow-md hover:bg-gray-50 transition duration-300 text-sm lg:text-base">Rejestracja</a>
                @endauth
            </div>
        ...
        <div x-show="open" @click.outside="open = false" x-transition.duration.200ms class="lg:hidden pb-3">
            <div class="flex flex-col space-y-3">
                <a href="{{ route('landing') }}#doctors"
                    class="text-primaryDark hover:text-primary transition text-sm">Lekarze</a>
                <a href="{{ route('landing') }}#services"
                    class="text-primaryDark hover:text-primary transition text-sm">Usługi</a>
                <a href="{{ route('landing') }}#reviews"
                    class="text-primaryDark hover:text-primary transition text-sm">Opinie</a>
                <a href="{{ route('landing') }}#faq"
                    class="text-primaryDark hover:text-primary transition text-sm">FAQ</a>
